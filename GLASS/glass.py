...
class GLASS(torch.nn.Module):
    ...
    def load(
        ...
        print(f"[DEBUG load] meta_epochs={meta_epochs}, eval_epochs={eval_epochs}")
    ...
    def trainer(self, training_data, val_data, name):
        print(f"[DEBUG trainer] about to run for {self.meta_epochs} epochs (eval every {self.eval_epochs})")
        ...
        for i_epoch in pbar:
            ...
            if (i_epoch + 1) % self.eval_epochs == 0:
                _, images, scores, segmentations, labels_gt, masks_gt = self.predict(val_data)
                ...
                eval_path = './results/eval/' + name + '/all_epochs/epoch_' + str(i_epoch)
                train_path = './results/training/' + name + '/'

                shutil.rmtree(eval_path, ignore_errors=True)
                shutil.copytree(train_path, eval_path)

                best_eval_path = './results/eval/' + name + '/best_epoch/epoch_' + str(i_epoch + 1)
                ...
                if best_record is None:
                    ...
                    shutil.rmtree(best_eval_path, ignore_errors=True)
                    shutil.copytree(train_path, best_eval_path)
    ...
    def tester(self, test_data, name):
        ...
        ckpt_paths = glob.glob(self.ckpt_dir + '/ckpt_best*')
        if len(ckpt_paths) != 0:
            # load best checkpoint
            state_dict = torch.load(ckpt_paths[0], map_location=self.device)
            ...
            # run inference
            img_paths, images, scores, segmentations, labels_gt, masks_gt = self.predict(test_data)

            # prepare folder for annotated images
            annotated_dir = os.path.join(self.model_dir, name, "annotated_scores")
            os.makedirs(annotated_dir, exist_ok=True)

            # prepare CSV to save image names and scores
            csv_path = os.path.join(self.model_dir, name, "image_scores.csv")
            with open(csv_path, mode='w', newline='') as csv_file:
                writer = csv.writer(csv_file)
                writer.writerow(["Image Name", "Score"])  # Header

                # annotate each image with its anomaly score and write to CSV
                for path, score in zip(img_paths, scores):
                    img = cv2.imread(path)
                    if img is None:
                        continue  # skip if image failed to load

                    # overlay the score in red text
            ...
            # flatten for threshold search
            seg_flat = np.array(segmentations).ravel()
            gt_flat  = np.array(masks_gt).ravel().astype(int)

            # find best threshold for F1
            best_threshold, best_precision, best_recall = metrics.compute_best_pr_re(gt_flat, seg_flat)

            # compute F2 at that threshold
            pred_flat = (seg_flat >= best_threshold).astype(int)
            f2_score  = fbeta_score(gt_flat, pred_flat, beta=2)

            # extract epoch number from filename
            epoch = int(ckpt_paths[0].split('_')[-1].split('.')[0])

        else:
            # no checkpoint found: zero everything
        ...
        return image_auroc, image_ap, pixel_auroc, pixel_ap, pixel_pro, f2_score, best_threshold, epoch
    ...
    #         # annotate each image with its anomaly score
    #         for path, score in zip(img_paths, scores):
    #             img = cv2.imread(path)
    #             if img is None:
    #                 continue  # skip if image failed to load
    ...
    def _evaluate(self, images, scores, segmentations, labels_gt, masks_gt, name, path='training'):
        ...
        # convert scores to a 1D numpy array so we can index it
        scores_arr = np.squeeze(np.array(scores))

        # compute the usual image‐ and pixel‐level metrics
        image_scores = metrics.compute_imagewise_retrieval_metrics(scores_arr, labels_gt, path)
        image_auroc, image_ap = image_scores["auroc"], image_scores["ap"]

        if len(masks_gt) > 0:
            segmentations = np.array(segmentations)
            pixel_scores = metrics.compute_pixelwise_retrieval_metrics(segmentations, masks_gt, path)
            pixel_auroc, pixel_ap = pixel_scores["auroc"], pixel_scores["ap"]
            pixel_pro = (
                metrics.compute_pro(np.squeeze(np.array(masks_gt)), segmentations)
                if path == 'eval' else 0.0
            )
        else:
            # no masks → return early
            return image_auroc, image_ap, -1.0, -1.0, -1.0

        # prepare output dir and make sure it's empty
        full_out = os.path.join('./results', path, name)
        annotated_dir = os.path.join(full_out, 'annotated')
        utils.del_remake_dir(annotated_dir, del_flag=False)

        defects = np.array(images)
        targets = np.array(masks_gt)

        # build gamma LUT (γ > 1 compresses mid values)
        gamma = 15
        ...
        for i in range(len(defects)):
            ...
            # resize and scale mask to 0–255
            mask_gray = cv2.resize(
                segmentations[i],
                (defect.shape[1], defect.shape[0])
            )
            mask_uint8 = (mask_gray * 255).astype('uint8')

            # gamma-correct, then colorize
            mask_gamma = cv2.LUT(mask_uint8, lut)
            mask_color = cv2.applyColorMap(mask_gamma, cv2.COLORMAP_JET)

            # stack defect / gt mask / heatmap
            img_up = np.hstack([defect, target, mask_color])
            img_up = cv2.resize(img_up, (256 * 3, 256))

            # overlay the image‐level score in red
            score_txt = f"Score: {scores_arr[i]:.4f}"
            cv2.putText(
                img_up,
                score_txt,
                org=(10, img_up.shape[0] - 10),
                fontFace=cv2.FONT_HERSHEY_SIMPLEX,
                fontScale=0.8,
                color=(0, 0, 255),      # BGR red
                thickness=2,
            ...
            # save
            out_fn = os.path.join(annotated_dir, f"{str(i+1).zfill(3)}.png")
            cv2.imwrite(out_fn, img_up)
    ...
    def predict(self, test_dataloader):
        ...
        return img_paths, images, scores, masks, labels_gt, masks_gt
